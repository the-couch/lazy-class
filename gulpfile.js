...
gulp.task('js', () => {
  return rollup
    ...
    .then(bundle => {
      ...
      const dist = 'dist/lazy-class.min.js'

      // write the JS and sourcemap
      fs.appendFileSync(dist, files.code)
      fs.writeFileSync('dist/maps/lazy-class.js.map', files.map.toString())
    })
})
...
gulp.task('watch', () => {
  gulp.watch('src/lazy-class.js', ['js', reload])
})
